...
namespace HA
{
    public class EventComponent : BaseComponent
    {
        ...
        private void OnListenerAdding(GameEventType eventType, Delegate callBack)
        {
            ...
            {
                HADebug.LogWarningFormat("尝试为事件[{0}]添加不同类型的委托，当前事件所对应的委托是[{1}]，要添加的委托类型为[{2}]", eventType, d.GetType(), callBack.GetType());
        ...
        private void OnListenerRemoving(GameEventType eventType, Delegate callBack)
        {
            if (_eventDic.ContainsKey(eventType))
            {
                ...
                if (d == null)
                {
                    HADebug.LogWarningFormat("移除监听错误：事件[{0}]没有对应的委托", eventType);
                    throw new Exception(string.Format("移除监听错误：事件[{0}]没有对应的委托", eventType));
                }
                else if (d.GetType() != callBack.GetType())
                {
                    HADebug.LogWarningFormat("移除监听错误：尝试为事件[{0}]移除不同类型的委托，当前委托类型为[{1}]，要移除的委托类型为[{2}]", eventType, d.GetType(), callBack.GetType());
                    throw new Exception(string.Format("移除监听错误：尝试为事件[{0}]移除不同类型的委托，当前委托类型为[{1}]，要移除的委托类型为[{2}]", eventType, d.GetType(), callBack.GetType()));
                }
            }
            else
            {
                HADebug.LogWarningFormat("移除监听错误：没有事件码[{0}]", eventType);
        ...
        public void Broadcast(GameEventType eventType) 
        {
            ...
            if (_eventDic.TryGetValue(eventType, out d))
            {
                ...
                {
                    HADebug.LogWarningFormat("广播事件错误：事件[{0}]对应委托具有不同的类型", eventType);
